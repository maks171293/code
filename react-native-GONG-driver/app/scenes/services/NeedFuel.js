...
class Fuel extends React.Component {
        ...
        setTimeout(() => {
            ...
            API.getRateByService({
                ...
                if (response instanceof Error) {
                    ...
                    console.log(strings['server error'],response);
                    return;
                }

                if (response.error) {
                    this.setState({errorMessage: strings['something went wrong, try again later']});
                    console.log(strings['not server error'], response);
                    return;
                }
    ...
    _doRouteAfterSubmit() {
        if (this.props.registrationProcess) {
            Actions.towService({service: {id: this.props.service.id+1}, registrationProcess: this.props.registrationProcess});
        } else {
            Actions.rates();
        }
    }
    ...
    renderFields() {
        return this.state.isServiceSupported && this.state.fields.map((field, index) => {
            //console.log(this.refs[(field.id+1)]);
            return (
                <TouchableOpacity
                    key={field.id}
                ...
                >
                    ...
                    <TextInput
                        returnKeyType={(index < this.state.fields.length - 1) ? "next" : "done"}
                        onSubmitEditing={() => {
        ...
        return (
            ...
            >
                <TouchableWithoutFeedback onPress={()=> dismissKeyboard()}>
                    <View style={{flex: 1}}>
                        <View style={{flex: 1}}>
                            <Text style={LayoutStyle.h1}>{strings["Setup fuel rates"]}</Text>
                        ...
                        <Text style={{color: '#949494', textAlign: 'center', fontSize: 8}}>
                            {strings["X liters of fuel should be delivered and included in price."]}
                        </Text>
