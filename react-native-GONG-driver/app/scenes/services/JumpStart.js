...
class JumpStart extends React.Component {
    ...
    _doRouteAfterSubmit() {
        if (this.props.registrationProcess) {
            Actions.flatTiresService({service: {id: this.props.service.id+1}, registrationProcess: this.props.registrationProcess});
        } else {
            Actions.rates();
        }
    }
        ...
        return (
            ...
            >
                <TouchableWithoutFeedback onPress={()=> dismissKeyboard()}>
                    <View style={{flex: 1}}>
                        <View style={{flex: 1}}>
                            <Text style={LayoutStyle.h1}>{strings["Setup jump start rates"]}</Text>
