...
const GooglePlacesAutocomplete = React.createClass({
    ...
    getDefaultProps() {
        return {
            ...
            getDefaultValue: () => '',
            timeout: 20000,
            onTimeout: () => console.warn(strings['google places autocomplete: request timeout']),
            query: {
                key: 'missing api key',
                language: 'en',
                types: 'geocode',
            ...
            currentLocationLabel: strings['Current location'],
        ...
        fetch('https://maps.googleapis.com/maps/api/place/details/json?' + Qs.stringify({
            ...
            .then(response => {
                if (response.result) {
                    ...
                    this.setAddressText(response.result.formatted_address);
                }
            })
            ...
            request.onreadystatechange = () => {
                ...
                if (request.status === 200) {
                    ...
                    if (responseJSON.status === 'OK') {
                        ...
                        this._disableRowLoaders();
                        console.warn(strings['google places autocomplete: '] + responseJSON.status);
                    }
                } else {
                    this._disableRowLoaders();
                    console.warn(strings['google places autocomplete: request could not be completed or has been aborted']);
                }
            };
            ...
            request.onreadystatechange = () => {
                ...
                if (request.status === 200) {
                    ...
                    if (typeof responseJSON.error_message !== 'undefined') {
                        console.warn(strings['google places autocomplete: '] + responseJSON.error_message);
                    }
                } else {
    ...
    _renderRow(rowData = {}, sectionID, rowID) {
        ...
        return (
            <ScrollView
                ...
                >
                    <View
                        style={[defaultStyles.row, this.props.styles.row, rowData.isPredefinedPlace ? this.props.styles.specialItemRow : {}]}>
                        <Text
    ...
    render() {
        ...
        return (
            <View>
                ...
                <View style={{zIndex: 2222, position: 'absolute'}}>
                    {this._getListView()}
                </View>
            </View>
        );
